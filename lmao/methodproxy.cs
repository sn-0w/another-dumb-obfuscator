    ...
    public static class proxymethod
    {
        ...
        public static MethodDef GenerateSwitch(MemberRef original, ModuleDef md)
        {
            try
            {
                ...
                MethodDef meth = new MethodDefUser("Sussy baka "+rand.Next(0, int.MaxValue), MethodSig.CreateStatic(original.MethodSig.RetType, type.ToArray()), methImplFlags, methFlags)
                {
                    Body = new CilBody()
                };
        ...
        public static void prxmeth(ModuleDef module)
        {
            ScanMemberRef(module);
            //for (var typecount = 0; typecount < module.GetTypes().Count(); typecount++)
            foreach (var type in module.GetTypes())
            {
               // var type = module.GetTypes().ToArray()[typecount];
                if (type.IsGlobalModuleType) continue;
                //for (var methodcount = 0; methodcount < type.Methods.Count(); methodcount++)
                foreach (var method in type.Methods.ToArray())
                {
                    //var method = type.Methods.ToArray()[methodcount];
                    if (!method.HasBody || method.Name.Contains("Sussy baka ")) continue;
                    var instr = method.Body.Instructions;
                    for (var i = 0; i < instr.Count; i++)
                    {
                            //Console.WriteLine(typecount * 100 / module.GetTypes().Count() + "%");
                        if (method.Body.Instructions[i].OpCode != OpCodes.Call) continue;
                        try
                        {
                            var original = (MemberRef)method.Body.Instructions[i].Operand;
